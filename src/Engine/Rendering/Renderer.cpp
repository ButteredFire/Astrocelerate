...
Renderer::Renderer(const Ctx::VkRenderDevice *renderDeviceCtx, const Ctx::VkWindow *windowCtx, std::shared_ptr<VkWindowManager> windowMgr, std::shared_ptr<VkCommandManager> commandMgr, std::shared_ptr<VkSyncManager> syncMgr, std::shared_ptr<UIRenderer> uiRenderer, std::shared_ptr<RenderSystem> renderSystem):
    ...
    */
    ...
    if (imgAcquisitionResult != VK_SUCCESS) {
        if (imgAcquisitionResult == VK_ERROR_OUT_OF_DATE_KHR || imgAcquisitionResult == VK_SUBOPTIMAL_KHR) {
            m_windowManager->recreateSwapchain(nullptr, nullptr);
            return;
        }
    ...
    auto renderBuf = m_commandManager->beginRenderBuffer(m_currentFrame, imageIndex, m_graphicsCommandBuffers[m_currentFrame]);
    {
        if (!m_secondaryCmdBufsStageNONE.empty()) {
            vkCmdExecuteCommands(renderBuf.buffer, m_secondaryCmdBufsStageNONE.size(), m_secondaryCmdBufsStageNONE.data());
            m_secondaryCmdBufsStageNONE.clear();
        }
    ...
    VkPipelineStageFlags waitStages[SIZE_OF(waitSemaphores)] = {
    ...
    uint32_t imgIndices[SIZE_OF(swapChains)] = {
        imageIndex
    };

    presentationInfo.swapchainCount = SIZE_OF(swapChains);
    presentationInfo.pSwapchains = swapChains;
    presentationInfo.pImageIndices = imgIndices;
