...
void PhysicsSystem::cacheECSData() {
	m_generalData.clear();
	m_propData.clear();
	m_identifierData.clear();

	// General data
	auto generalView = m_ecsRegistry->getView<CoreComponent::Transform, PhysicsComponent::RigidBody>();
	auto propView = m_ecsRegistry->getView<PhysicsComponent::Propagator, CoreComponent::Transform, PhysicsComponent::RigidBody>();
	...
	for (int i = 0; i < m_generalData.size(); i++) {
		...
		std::get<CoreComponent::Identifiers>(m_identifierData[i]) = m_ecsRegistry->getComponent<CoreComponent::Identifiers>(id);
	}
}


void PhysicsSystem::syncECSData() {
	// General data
	...
	for (int i = 0; i < m_identifierData.size(); i++) {
		...
		if (identifier.entityType == CoreComponent::Identifiers::EntityType::STAR) {
			RenderComponent::PointLight &pointLight = m_ecsRegistry->getComponent<RenderComponent::PointLight>(entityID);

			auto &&[_, transform, _1] = m_generalData[i];
			pointLight.position = SpaceUtils::ToRenderSpace_Position(transform.position);

			m_ecsRegistry->updateComponent(entityID, pointLight);
		}
	}


	// Update epoch
	auto [id, coordSys] = m_ecsRegistry->getView<PhysicsComponent::CoordinateSystem>()[0];
	{
...
void PhysicsSystem::updateGeneralBodies(const double dt, const double et) {
	...
	for (size_t i = 0; i < m_generalData.size(); i++) {
...

		{
			...
			//if (m_ecsRegistry->hasComponent<PhysicsComponent::Propagator>(targetEntityID))
			//	continue;
		}
		...
		//m_ecsRegistry->updateComponent(targetEntityID, targetTransform);
		//m_ecsRegistry->updateComponent(targetEntityID, targetRigidBody);
	}
}
