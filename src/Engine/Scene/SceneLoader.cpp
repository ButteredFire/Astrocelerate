...
#define info(entityName, componentType) "Entity " + enquote((entityName)) + ", component " + enquote((componentType)) + ": "
    ...
    for (const auto &entityNode : sceneRoot) {
        ...
        if (StringUtils::BeginsWith(originalEntityName, YAMLScene::Body_Prefix)) {
            ...
            m_ecsRegistry->addComponent(newEntity.id, identifiers);
            m_ecsRegistry->addComponent(newEntity.id, transform);
            m_ecsRegistry->addComponent(newEntity.id, rigidBody);
            m_ecsRegistry->addComponent(newEntity.id, shapeParams);
            m_ecsRegistry->addComponent(newEntity.id, meshRenderable);
        ...
        for (const auto &componentNode : entityNode[YAMLScene::Entity_Components]) {
            ...
            if (componentType == YAMLScene::Core_Transform) {
                ...
                m_ecsRegistry->addComponent(newEntity.id, propagator);
            }
            ...
            // ----- SPACECRAFT -----
                ...
                m_ecsRegistry->addComponent(newEntity.id, spacecraft);
            }
                ...
                m_ecsRegistry->addComponent(newEntity.id, thruster);
            }
            ...
            // ----- RENDERING -----
                ...
                if (renderableNode[YAMLData::Render_MeshRenderable_MeshPath]) {
                    ...
                    //
                    //                m_ecsRegistry->addComponent(newEntity.id, renderableCopy);
                    //            }
                    //        )
                    //    }
                    //);
