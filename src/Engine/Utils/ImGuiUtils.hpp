...
namespace ImGuiUtils {
	...
	*/
    inline void BoldText(const char* fmt, ...) {
		LOG_ASSERT(g_guiCtx.Font.bold, "Cannot embolden text " + enquote(fmt) + ": The bold font has not been loaded!");
	...
	*/
	inline void ItalicText(const char* fmt, ...) {
		LOG_ASSERT(g_guiCtx.Font.italic, "Cannot italicize text " + enquote(fmt) + ": The italic font has not been loaded!");
		ImGui::PushFont(g_guiCtx.Font.italic);
		va_list args;
		va_start(args, fmt);
	...
	*/
	inline void LightText(const char* fmt, ...) {
		LOG_ASSERT(g_guiCtx.Font.light, "Cannot render light text " + enquote(fmt) + ": The bold font has not been loaded!");
		ImGui::PushFont(g_guiCtx.Font.light);
		va_list args;
		va_start(args, fmt);
		ImGui::TextWrappedV(fmt, args);
		va_end(args);
