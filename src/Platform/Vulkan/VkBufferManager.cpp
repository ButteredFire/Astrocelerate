...
VkBufferManager::VkBufferManager(VkCoreResourcesManager *coreResources, VkSwapchainManager *swapchainMgr) :
	...
	m_depParentID = m_cleanupManager->createCleanupTask(CleanupTask{
		.caller = __FUNCTION__
	});
...
void VkBufferManager::bindEvents() {
	...
	m_eventDispatcher->subscribe<UpdateEvent::SessionStatus>(selfIndex,
		[this, selfIndex](const UpdateEvent::SessionStatus &event) {
			...
			case RESET:
				// Destroy buffers
				m_cleanupManager->executeCleanupTask(m_depParentID, false);
...
CleanupID VkBufferManager::CreateBuffer(VkBuffer& buffer, VkDeviceSize bufferSize, VkBufferUsageFlags usageFlags, VmaAllocation& bufferAllocation, VmaAllocationCreateInfo bufferAllocationCreateInfo) {
	...
	CleanupID bufferTaskID = cleanupManager->createCleanupTask(bufTask);
...
void VkBufferManager::createGlobalVertexBuffer(const std::vector<Geometry::Vertex>& vertexData) {
	...
	m_cleanupManager->addTaskDependency(taskID, m_depParentID);
	...
	*/
	...
	for (size_t i = 0; i < SimulationConst::MAX_FRAMES_IN_FLIGHT; i++) {
		...
		*/
		...
		m_cleanupManager->addTaskDependency(globalBufCleanupID, m_depParentID);
		...
		m_cleanupManager->addTaskDependency(objectBufCleanupID, m_depParentID);
		...
		m_cleanupManager->createCleanupTask(task, globalBufCleanupID);
		...
		m_cleanupManager->createCleanupTask(task, objectBufCleanupID);
	}
}
...
void VkBufferManager::createMatParamsUniformBuffer() {
	...
	m_cleanupManager->addTaskDependency(cleanupTaskID, m_depParentID);
	...
	m_cleanupManager->createCleanupTask(task, cleanupTaskID);
...
void VkBufferManager::updateGlobalUBO(uint32_t currentImage, const glm::dvec3 &renderOrigin) {
	...
	// View
	auto pointLightView = m_ecsRegistry->getView<RenderComponent::PointLight>();
...
void VkBufferManager::updateObjectUBOs(uint32_t currentImage, const glm::dvec3& renderOrigin) {
	auto view = m_ecsRegistry->getView<CoreComponent::Transform, PhysicsComponent::RigidBody, RenderComponent::MeshRenderable, TelemetryComponent::RenderTransform>();
	...
	for (auto&& [entity, transform, rigidBody, meshRenderable, renderT] : view) {
		...
		//m_ecsRegistry->updateComponent(entity, refFrame);
		...
		*/
		...
		m_ecsRegistry->updateComponent(entity, renderT);
