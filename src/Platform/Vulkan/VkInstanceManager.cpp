...
CleanupTask VkInstanceManager::createVulkanInstance(VkInstance &instance) {
    ...
    bool enableDebugLogs = (IN_DEBUG_MODE || g_appCtx.Config.debugging_VkValidationLayers);
    if (enableDebugLogs)
        addVulkanExtensions({
...
CleanupTask VkInstanceManager::createDebugMessenger(VkDebugUtilsMessengerEXT &dbgMessenger, VkInstance instance) {
    ...
    task.cleanupConditions = { IN_DEBUG_MODE || g_appCtx.Config.debugging_VkValidationLayers };
