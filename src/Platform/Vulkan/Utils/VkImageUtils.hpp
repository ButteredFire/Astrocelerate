/* VkImageUtils - Manages Vulkan image resource creation.
*/
#pragma once
...
namespace VkImageUtils {
	...
	*/
	inline ResourceID CreateImage(const Ctx::VkRenderDevice *renderDevice, VkImage &image, VmaAllocation &imgAllocation, VmaAllocationCreateInfo &imgAllocationCreateInfo, uint32_t width, uint32_t height, uint32_t depth, VkFormat imgFormat, VkImageTiling imgTiling, VkImageUsageFlags imgUsageFlags, VkImageType imgType) {
		...
		*/
		...
		imgTask.cleanupFunc = [renderDevice, image, imgAllocation]() {
			vmaDestroyImage(renderDevice->vmaAllocator, image, imgAllocation);
		};

		uint32_t taskID = cleanupManager->createCleanupTask(imgTask);
	...
	/* Creates an image view.
		@param logicalDevice: The logical device handle.
		...
		@note This function assumes the garbage collector service has already been registered.
	*/
	inline ResourceID CreateImageView(VkDevice logicalDevice, VkImageView &imageView, VkImage &image, VkFormat imgFormat, VkImageAspectFlags imgAspectFlags, VkImageViewType viewType, uint32_t levelCount, uint32_t layerCount) {
		...
		*/
		...
		uint32_t taskID = cleanupManager->createCleanupTask(task);
	...
	/* Creates a framebuffer.
		...
		@note This function assumes the garbage collector service has already been registered.
	*/
	inline ResourceID CreateFramebuffer(VkDevice logicalDevice, VkFramebuffer &framebuffer, VkRenderPass &renderPass, std::vector<VkImageView> attachments, uint32_t width, uint32_t height) {
		...
		VkResult result = vkCreateFramebuffer(logicalDevice, &bufferCreateInfo, nullptr, &framebuffer);
